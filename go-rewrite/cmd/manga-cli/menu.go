...
func mainMenu(cfg util.Config) error {
	...
	for {
		...
		entries = append(entries,
			...
			menuEntry{label: "⬇️  " + ui.Bold(pad("download")) + " " + ui.Dim("save chapters as CBZ · ZIP · PDF"),
				run: func() error { return downloadFlow(cfg, "", dlOpts{}) }, pause: true},
			menuEntry{label: "📚  " + ui.Bold(pad("library")) + " " + ui.Dim("read your downloads offline"),
				run: func() error { return libraryFlow(cfg) }},
			menuEntry{label: "🔄  " + ui.Bold(pad("sync")) + " " + ui.Dim("grab new chapters for your follows"),
